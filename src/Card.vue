  ...
  this.update = function(thisElement, binding) {
    ...
    for (let el of params.bottom ? children.reverse() : children) {
      el.children[params.adjust].setAttribute(
        'height',
        el.children[params.check].firstChild.clientHeight + params.padding
      );

      let translate = heightAcc + params.offset || 0;
      heightAcc +=
        el.children[params.check].firstChild.clientHeight + params.margin;
      if (params.bottom) {
...
export default {
  ...
  computed: {
    template() {
      ...
      let cardType =
        this.type in deckTemplates
          ? this.type
          : this.type.replace(/-back$/, '');
      return deckTemplates[cardType].replace(
